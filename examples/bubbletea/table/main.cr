...
class TableModel
  ...
  BASE_STYLE = Lipgloss::Style.new.border(Lipgloss::Border.normal).border_foreground(Lipgloss::Color.indexed(240))

  getter table : TC::Table

  def initialize
    columns = [
      TC::Table::Column.new("Rank", 4),
      TC::Table::Column.new("City", 10),
      TC::Table::Column.new("Country", 10),
      TC::Table::Column.new("Population", 10),
    ]
    ...
    t = TC::Table.new(columns: columns, rows: table_rows, height: 7)
    t.focus

    styles = TC::Table.default_styles
    t.header_style = styles.header.border(Lipgloss::Border.normal, true).border_foreground(Lipgloss::Color.indexed(240)).border_bottom(true).bold(false)
  ...
  def update(msg : Term2::Msg) : {Term2::Model, Term2::Cmd}
    ...
    when Term2::KeyMsg
      ...
      when "enter"
        ...
        if row && row.size > 1
          return {self, Term2::Cmds.printf("Let's go to %s!", row[1])}
        end
      end
    end
