  ...
  module StyleTable
      ...
      def initialize(@table_width : Int32, @table_height : Int32, @headers : Array(String), rows : Array(Array(String)))
        ...
        @all_rows.each do |row|
          row.each_with_index do |cell, idx|
            cell_len = Term2::Text.width(cell)
            if idx >= @columns.size
        ...
        @columns.each_with_index do |col, idx|
          widths = col.rows.map { |r| Term2::Text.width(r[idx]) }
          new_col = @columns[idx]
    ...
    class Table
      ...
      def string : String
        ...
        if @data.rows > 0
          if @use_manual_height
            top_height = prefix.empty? ? 0 : (Term2::Text.height(prefix) - 1)
            available = @height - (top_height + Term2::Text.height(bottom))
      ...
      private def resize : Nil
        ...
        all_rows.each_with_index do |row, i|
          ...
          row.each_index do |j|
            ...
            content_width = Term2::Text.width(row[j])
      ...
      private def construct_row(index : Int32, is_overflow : Bool) : String
        ...
        cells.map!(&.rstrip('\n'))
        sb << Term2.join_horizontal(Term2::Position::Top, cells)
        sb << "\n"
    ...
    def self.truncate(text : String, width : Int32, tail : String = "") : String
      ...
      target = [width - Term2::Text.width(tail), 0].max
      visible = String::Builder.new
      current = 0
      text.each_grapheme do |g|
        g_width = Term2::Text.width(g.to_s)
