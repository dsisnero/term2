require "./spec_helper"
require "../src/term2"

describe Lipgloss::Style do
  describe "Fluent API" do
    it "sets foreground color" do
      style = Lipgloss::Style.new.foreground(Lipgloss::Color::RED)
      style.foreground_color.should eq(Lipgloss::Color::RED)
    end

    it "sets background color" do
      style = Lipgloss::Style.new.background(Lipgloss::Color::BLUE)
      style.background_color.should eq(Lipgloss::Color::BLUE)
    end

    it "sets text attributes" do
      style = Lipgloss::Style.new.bold(true).italic(true).underline(true)
      style.bold?.should be_true
      style.italic?.should be_true
      style.underline?.should be_true
    end

    it "sets padding" do
      style = Lipgloss::Style.new.padding(1, 2, 3, 4)
      style.padding_top.should eq(1)
      style.padding_right.should eq(2)
      style.padding_bottom.should eq(3)
      style.padding_left.should eq(4)
    end

    it "sets margin" do
      style = Lipgloss::Style.new.margin(1)
      style.margin_top.should eq(1)
      style.margin_right.should eq(1)
      style.margin_bottom.should eq(1)
      style.margin_left.should eq(1)
    end

    it "sets width and height" do
      style = Lipgloss::Style.new.width(10).height(5)
      style.width_value.should eq(10)
      style.height_value.should eq(5)
    end

    it "sets alignment" do
      style = Lipgloss::Style.new.align(Lipgloss::Position::Center)
      style.align_value.should eq(Lipgloss::Position::Center)
    end
  end

  describe "Rendering" do
    it "renders basic text" do
      style = Lipgloss::Style.new
      output = style.render("Hello")
      output.should eq("Hello")
    end

    it "renders with padding" do
      style = Lipgloss::Style.new.padding(1, 2)
      output = style.render("Hello")
      lines = output.split('\n')
      lines.size.should eq(3)
      lines[0].should eq(" " * 9)
      lines[1].should eq("  Hello  ")
      lines[2].should eq(" " * 9)
    end

    it "renders with margin" do
      style = Lipgloss::Style.new.margin(1, 0)
      output = style.render("Hello")
      lines = output.split('\n')
      lines.size.should eq(3)
      lines[0].should eq(" " * 5)
      lines[1].should eq("Hello")
      lines[2].should eq(" " * 5)
    end
  end

  describe "Copying" do
    it "creates an independent copy" do
      original = Lipgloss::Style.new.bold(true).foreground(Lipgloss::Color::RED)
      copy = original.copy

      copy.italic(true)

      original.italic?.should be_false
      copy.italic?.should be_true
      copy.bold?.should be_true
    end
  end
end

describe Lipgloss::Color do
  it "creates named colors" do
    color = Lipgloss::Color::RED
    color.type.should eq(Lipgloss::Color::Type::Named)
  end

  it "creates indexed colors" do
    color = Lipgloss::Color.new(Lipgloss::Color::Type::Indexed, 128)
    color.type.should eq(Lipgloss::Color::Type::Indexed)
    color.value.should eq(128)
  end

  it "creates RGB colors from hex" do
    color = Lipgloss::Color.from_hex("#FF5500")
    color.type.should eq(Lipgloss::Color::Type::RGB)
    color.value.should eq({255, 85, 0})
  end

  it "handles short hex colors" do
    color = Lipgloss::Color.from_hex("#F50")
    color.type.should eq(Lipgloss::Color::Type::RGB)
    color.value.should eq({255, 85, 0})
  end
end
