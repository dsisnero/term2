...
describe Term2::Components::Timer do
  it "initializes running" do
    timer = Term2::Components::Timer.new(5.seconds)
    timer.running?.should be_true
    timer.timed_out?.should be_false
  end

  it "counts down" do
    timer = Term2::Components::Timer.new(2.seconds, 1.second)

    # Simulate tick
    msg = Term2::Components::Timer::TickMsg.new(timer.id, 0, false)
  ...
  it "can be stopped and started" do
    ...
    # Stop
    msg = Term2::Components::Timer::StartStopMsg.new(timer.id, false)
    timer, _ = timer.update(msg)
    timer.running?.should be_false

    # Start
    msg = Term2::Components::Timer::StartStopMsg.new(timer.id, true)
    timer, cmd = timer.update(msg)
    timer.running?.should be_true
    cmd.should_not be_nil
  end
