...
describe Term2::Bubbles::Table do
  it "initializes with data" do
    cols = [
      Term2::Bubbles::Table::Column.new("ID", 5),
      Term2::Bubbles::Table::Column.new("Name", 10),
    ]
    rows = [
      ["1", "Alice"],
      ["2", "Bob"],
    ...
    table = Term2::Bubbles::Table.new(cols, rows)
    table.rows.size.should eq 2
    table.columns.size.should eq 2
  end

  it "initializes with DSL" do
    table = Term2::Bubbles::Table.new(width: 50, height: 10) do |tbl|
      tbl.column "ID", 5
      tbl.column "Name", 10
      tbl.row "1", "Alice"
      tbl.row "2", "Bob"
  ...
  it "navigates" do
    cols = [Term2::Bubbles::Table::Column.new("ID", 5)]
    rows = [["1"], ["2"], ["3"]]
    table = Term2::Bubbles::Table.new(cols, rows)
    table.focus
